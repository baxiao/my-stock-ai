...
if "access_password" not in st.secrets or "deepseek_api_key" not in st.secrets:
    st.error("❌ 请在 Secrets 中配置 access_password 与 deepseek_api_key"); st.stop()

# %% 2. 状态初始化
defaults = dict(logged_in=False, ai_cache="", last_data=None, last_code="",
                auto_refresh=False, stop_loss=0, take_profit=0)
...
if not st.session_state.logged_in:
    st.title("🔐 文哥哥私人终端")
    if st.text_input("访问密钥", type="password") == st.secrets.access_password:
...
def get_stock_data(code: str):
    try:
        ...
        df = df.rename(columns=str.lower)          # 统一小写
        last = df.iloc[-1]
...
def four_lights(data):
    if data is None: return {"trend":"⚪","money":"⚪","sentiment":"⚪","safety":"⚪"}
    df, fund = data["df"], data["fund"]
    close = df["收盘"]
    ma5, ma20 = close.tail(5).mean(), close.tail(20).mean()
    trend = "🔴" if ma5 > ma20 else "🟢"
    if abs(ma5 - ma20) / ma20 < 0.01: trend = "⚪"          # 无量横盘
    money = "🔴" if fund is not None and fund.get("主力净流入-净额", 0) > 0 else "🟢"
...
with st.sidebar:
    ...
    st.number_input("止损价", value=0., step=0.01, key="stop_loss")
    st.number_input("目标价", value=0., step=0.01, key="take_profit")
    if st.button("🔴 退出系统"):
...
with tab2:
    ...
    def render():
        ...
        if data is None: ph.warning("连接中..."); return
        ...
        with ph.container():
            ...
            main = data["fund"]["主力净流入-净额"] if data["fund"] is not None else 0
            col2.metric("主力净额", f"{main/1e8:.2f} 亿", "多方" if main>0 else "空方")
...
st.divider()
st.caption("文哥哥专用 | %s" % datetime.now(CN_TZ).strftime("%Y-%m-%d %H:%M"))
